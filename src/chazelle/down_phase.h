...
///
/// Algorithm (Lemma 4.2):
///   Recurse on grades λ = p → ⌈βλ⌉ → ⌈β⌈βλ⌉⌉ → … → O(1).
///   At each grade, refine the current submap by filling in details within
///   each region:
...
///        as curve edges.
///     3. Use arc-cutting oracle to decompose arcs into O(λ) prior-grade
///        chains (§4.1: g(γ) = O(λ)).
